...
```{r}
x=c(NA,x)
c(mean(x),median(x),quantile(x,0.9))
``` 
...
```{r}
matrix(c(colMeans(C), apply(C_inv,MARGIN=1,FUN=median)),nrow=2,byrow=TRUE)
```
...
- What is the second argument of `max()` and what does it do? na.rm: include or exclude NA
...
:::
```{r}
grades[7,]<-c(7,55,'Pass',TRUE,6,'Male')
